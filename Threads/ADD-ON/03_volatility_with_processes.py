...
class Trader:
    ...
    def __init__(self, deals_path: str) -> None:
        ...
        # расчеты запускаем в мультипроцессинге
        with multiprocessing.Pool(len(self.tickers)) as p:
            p.map(self.__get_vol, self.tickers)
...
# запуск. Среднее время выполнения задачи - 4.18 сек Почему время выполнения задачи такое же как в однопоточном
# ????????
